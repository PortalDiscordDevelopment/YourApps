...
export function ModuleInjection(
	...
	const injections =
		optionsOrModuleName.map(
			e =>
				typeof e == "string"
				? {
						propertyName: e.replace(/-(\w)/g, m =>
							m[1].toUpperCase()
						),
						moduleName: e
				}
	...
	return createClassDecorator(
		<T extends { new (...args: any[]): {} }>(target: T) => {
			for (const { moduleName, propertyName } of injections) {
			...
			return createProxy(target, {
				construct: (ctor, args: unknown[]) => {
					...
					// Add the properties with the names specified in options.propertyName to the class
					for (const { moduleName, propertyName } of injections) {
