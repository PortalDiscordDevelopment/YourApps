...
@ApplyOptions<CommandOptions>({
	...
	description: 'Adds a role to the list of reward, remove, or required roles for a position',
	preconditions: [],
	slashOptions: {
		options: [
			{
				name: 'position',
				description: 'The positon to add a reward role to',
				type: 'STRING',
				autocomplete: true,
				required: true
			},
...
export class ConfigPositionsRolesAddCommand extends BotCommand {
	override async chatInputRun(interaction: CommandInteraction) {
		...
		const { position: positionName, role, type }: { position: string; role: Role; type: `${'reward'|'required'|'remove'}roles` } =
