...
import { Events, Listener, ListenerOptions, UnknownChatInputCommandPayload } from "@sapphire/framework";
...
export class CustomCommandsListener extends Listener<typeof Events.UnknownChatInputCommand> {
    override async run({ interaction }: UnknownChatInputCommandPayload) {
        ...
        if (!position) {
            this.container.logger.warn('An unrecognized chat input command was received, but there was no custom command to go along with it.')
            return
        }
        // Start application
